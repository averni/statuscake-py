# coding: utf-8

"""
    StatusCake API

    StatusCake API endpoints to manage monitoring resources.  # Authentication  Documentation on API authentication can be found [here](https://developers.statuscake.com/guides/api/authentication).  # Ratelimiting  Documentation on API ratelimiting can be found [here](https://developers.statuscake.com/guides/api/ratelimiting).  # Errors  Documentation on error handling can be found [here](https://developers.statuscake.com/guides/api/errors).  # Handling Input Parameters  Documentation on input parameters, including how to pass arrays to API endpoints can be found [here](https://developers.statuscake.com/guides/api/parameters). 

    The version of the OpenAPI document: 1.2.0
    Contact: support@statuscake.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from statuscake.models.pagespeed_test_history import PagespeedTestHistory

class TestPagespeedTestHistory(unittest.TestCase):
    """PagespeedTestHistory unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PagespeedTestHistory:
        """Test PagespeedTestHistory
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PagespeedTestHistory`
        """
        model = PagespeedTestHistory()
        if include_optional:
            return PagespeedTestHistory(
                data = [
                    statuscake.models.pagespeed_test_history_result.PagespeedTestHistoryResult(
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        filesize = 0, 
                        har_location = '', 
                        loadtime = 0, 
                        requests = 0, 
                        throttling = 'NONE', )
                    ],
                links = {
                    'key' : None
                    },
                metadata = {
                    'key' : None
                    }
            )
        else:
            return PagespeedTestHistory(
                data = [
                    statuscake.models.pagespeed_test_history_result.PagespeedTestHistoryResult(
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        filesize = 0, 
                        har_location = '', 
                        loadtime = 0, 
                        requests = 0, 
                        throttling = 'NONE', )
                    ],
                links = {
                    'key' : None
                    },
        )
        """

    def testPagespeedTestHistory(self):
        """Test PagespeedTestHistory"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
